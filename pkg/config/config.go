...
type Build struct {
	...
	Builder         string          `yaml:"builder,omitempty" json:"builder,omitempty" jsonschema:"enum=,enum=go,enum=rust,enum=zig,enum=bun,enum=deno,enum=node,enum=uv,enum=poetry,enum=prebuilt"`
...
// Telemetry configures OpenTelemetry tracing of the release pipeline.
//
// Traces are exported to the collector configured through the standard
// OTEL_* environment variables, and never to GoReleaser itself.
//
// Added in v2.18. Enterprise-only.
...
type DockerV2 struct {
	...
	// pro-only
	TemplatedExtraFiles []TemplatedExtraFileWithMode `yaml:"templated_extra_files,omitempty" json:"templated_extra_files,omitempty"`
	TemplatedDockerfile string                       `yaml:"templated_dockerfile,omitempty" json:"templated_dockerfile,omitempty"`
